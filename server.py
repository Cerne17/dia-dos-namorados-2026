...
def generate_next_question(api_key, previous_answers):
    ...
    if num_answers >= len(categories):
        return None  # All steps generated
    ...
    def do_POST(self):
        if self.path == '/api/results':
            ...
            try:
                ...
                print(f"[SERVER] Encontro planejado salvo em results.json")
                
                self.send_response(200)
                self.send_header('Content-Type', 'application/json')
                self.send_header('Access-Control-Allow-Origin', '*')
                self.end_headers()
                response = {"status": "success", "message": "Respostas salvas com sucesso."}
            ...
            try:
                ...
                print(f"[SERVER] Erro ao processar POST de geração de pergunta: {e}", file=sys.stderr)
                self.send_response(500)
                self.send_header('Content-Type', 'application/json')
                self.end_headers()
                response = {"status": "error", "message": str(e)}
